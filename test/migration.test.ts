...
// @ts-ignore
const stakingRewardsContract = contract(stakingRewardsAbi, "0xB5303c22396333D9D27Dc45bDcC8E7Fc502b4B32");
// @ts-ignore
const orbsTokenContract = contract(orbsTokenAbi, orbsToken);
let migrationManagerAddress;
...
describe("StakingMigrationContract", async () => {
  ...
  it("Add/remove migration destinations", async () => {
    ...
    await expectRevert(
        async () =>
            await stakingContract.methods.removeMigrationDestination(migrationContractAddress2).send({ from: migrationManagerAddress }),
        /staking contract doesn't exist/
    );
    await expectRevert(
        async () =>
            await stakingContract.methods.approvedStakingContracts(1).call(),
        /invalid opcode/
    );
  ...
  it("Should stake/unstake normally", async () => {
    ...
    await orbsTokenContract.methods.approve(stakingContractAddress, "99999999999999999999999999999999999999").send({from: OrbsWhale});
    ...
    await stakingContract.methods.removeMigrationDestination(migrationContractAddress).send({ from: migrationManagerAddress });
    await stakingContract.methods.unstake(testAmount).send({ from: OrbsWhale });
    await mineBlock(14 * 24 * 60 * 60);
    await stakingContract.methods.withdraw().send({ from: OrbsWhale });
    ...
    expect(await stakingContract.methods.getStakeBalanceOf(OrbsWhale).call()).bignumber.eq(stakeBalance.minus(testAmount.multipliedBy(2)));
  });
  ...
  it("Shouldn't affect guardian", async () => {
    ...
    expect(BN(newStakingRewardsBalance.delegatorStakingRewardsBalance)).bignumber.greaterThan(BN(stakingRewardsBalance.delegatorStakingRewardsBalance));
    const newGuardianStakingRewardsBalance = await stakingRewardsContract.methods.getStakingRewardsBalance(neoply).call();
    expect(BN(newGuardianStakingRewardsBalance.delegatorStakingRewardsBalance)).bignumber.greaterThan(BN(guardianStakingRewardsBalance.delegatorStakingRewardsBalance));
    expect(BN(newGuardianStakingRewardsBalance.guardianStakingRewardsBalance)).bignumber.greaterThan(BN(guardianStakingRewardsBalance.guardianStakingRewardsBalance));
  ...
  it("Shouldn't affect other delegators", async () => {
    const randomDelegator = "0x6498Ed4e7577584a59e621e42De4ED0dCE80c136";
    ...
    await orbsTokenContract.methods.approve(stakingContractAddress, "99999999999999999999999999999999999999").send({from: randomDelegator});
  ...
  it("Shouldn't allow direct call of acceptMigration", async () => {
    await expectRevert(() => migrationContract.methods.acceptMigration(OrbsWhale, BN(10e18)).send({ from: OrbsWhale }), /Unauthorized caller/);
  });
});
